...
import json, hashlib
R=Path(__file__).resolve().parents[2]; H=(R/'preview.html').read_text();O=R/'reports/v1.7/operating-browser';O.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
 ...
 pg.click('#instrument-open');check('One instrument panel includes all eleven primary tabs',pg.locator('.ix-tabs [role=tab]').count()==11)
 ...
 check('Export includes the complete operating and laboratory state',archive['engine']['schema']=='idfbi/remembering/1.7'and len(archive['engine']['operating']['operating']['cells'])==2)
