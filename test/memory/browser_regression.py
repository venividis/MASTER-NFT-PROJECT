...
import json, hashlib
R=Path(__file__).resolve().parents[2]; H=(R/'preview.html').read_text(); O=R/'reports/v1.7/regression';O.mkdir(exist_ok=True,parents=True)
...
with sync_playwright() as p:
 ...
 check('Original and instruments boot together',E("__idfbi.version==='1.2.0'&&__instruments.version==='1.7.0'"))
 ...
 download=dl.value;download.save_as(O/'browser-export.json');exported=json.loads((O/'browser-export.json').read_text());check('Export is a tagged unsigned local archive',exported['engine']['schema']=='idfbi/remembering/1.7')
