...
export class RegistryAdapter {
 ...
 async savedState(recovered){const c=await this.context(),installation=await this.installation(recovered.moduleKey,c);if(installation.stateHead===zero)return{installation,value:{},head:zero};const state=await sdk.recoverState({request:this.request,stateStore:c.stateStore,stateId:installation.stateHead,chainId:c.chainId,snapshot:c.snapshot,collection:c.collection,tokenId:c.tokenId,moduleKey:recovered.moduleKey});const value=boundedJSON(JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(state.bytes)),65536);return{installation,value,head:installation.stateHead,schema:state.record.schema};}
 async intent(kind,recovered,{value={},nextStateHead}={}){
  ...
  if(kind==='activate')recipe=sdk.encodeActivate(c,{releaseId:recovered.releaseId,expectedStateHead:installation.stateHead,nextStateHead:nextStateHead??installation.stateHead});
   ...
   recipe=sdk.encodeWriteState(c,{moduleKey:recovered.moduleKey,bytes:new TextEncoder().encode(sdk.canonicalJSON(value))});
  }
  else if(kind==='stageState')recipe=sdk.encodeStageState(c,{moduleKey:recovered.moduleKey,stateSchema:recovered.manifest.stateSchema,bytes:new TextEncoder().encode(sdk.canonicalJSON(value))});
