...
export function neighbors(i        )           { const x = i % SIZE, y = Math.floor(i / SIZE); return [[x-1,y],[x+1,y],[x,y-1],[x,y+1]].filter(([a,b]) => a >= 0 && a < SIZE && b >= 0 && b < SIZE).map(([a,b]) => b * SIZE + a) }
...
export function initialMatch(world = defaultWorld())        { return { world: validateWorld(world), cells: Array.from({length:81},(_,i)=>i===0?1:i===80?2:0), turn:1, moves:0, scores:[1,1], winner:null, log:[] } }
export function legalMoves(m       , player = m.turn)           { return m.winner !== null ? [] : m.cells.flatMap((v,i) => v === 0 && m.world.terrain[i] !== "wall" && neighbors(i).some(n => m.cells[n] === player) ? [i] : []) }
export function playMove(m       , cell        , player = m.turn)        {
  ...
  const next        = { ...m, cells:[...m.cells], scores:[...m.scores], moves:m.moves+1, turn:3-player, log:[...m.log,{player,cell}] }
...
export function botMove(m       )                     { return legalMoves(m).sort((a,b) => (m.world.terrain[b] === "prism" ? 100 : 0) - (m.world.terrain[a] === "prism" ? 100 : 0) || Math.abs(a%9-4)+Math.abs(Math.floor(a/9)-4)-Math.abs(b%9-4)-Math.abs(Math.floor(b/9)-4))[0] }
...
export function splitUnits(amount        , routes            )           {
